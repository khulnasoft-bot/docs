...
export function middleware(request: NextRequest) {
	...
	if (latestVersionOnlyMatch) {
		return NextResponse.redirect(`https://khulnasoft.com/docs`);
	}
	...
	if (
		...
		return NextResponse.redirect(
			`https://${versionOnlyMatch[1]}.khulnasoft.com/`
		);
	}
	...
	if (pathWithoutBase.startsWith(`/v/${docsConfig.DOCS_LATEST_VERSION}/`)) {
		return NextResponse.redirect(
			createRedirectUrl(
				request,
				`https://khulnasoft.com/docs/:slug*`,
				pathWithoutBase
			)
		);
	}
	...
	if (versionMatch) {
		return NextResponse.redirect(
			createRedirectUrl(
				request,
				'https://:version.khulnasoft.com/:slug*',
				pathWithoutBase
			)
		);
	}
